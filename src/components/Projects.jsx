...
const Projects = () => {
  ...
  const projectData = [
    {
      ...
      description:
        "A fully responsive modern store with high performance and smooth UI animations.",
    ...
    },
    {
      ...
      description:
        "Real-time analytics dashboard for monitoring user engagement and data visualization.",
  ...
  return (
    <section
      id="works"
      className="lg:py-20 md:py-12 py-6 bg-white dark:bg-[#08080a] transition-colors duration-700"
    >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          {projectData.map((project) => (
            <div key={project.id} className="group relative">
              {/* Image Container */}
              <div className="relative overflow-hidden rounded-2xl bg-gray-100 dark:bg-[#111116] border border-gray-200 dark:border-white/5 shadow-2xl transition-all duration-500 group-hover:shadow-indigo-500/10">
                <img
                  src={project.image}
                  alt={project.title}
                ...
                <div className="absolute inset-0 bg-indigo-900/80 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center gap-6">
                  <a
                    href={project.github}
                    className="p-4 bg-white/10 backdrop-blur-md rounded-full text-white hover:bg-white hover:text-indigo-900 transition-all active:scale-90"
                  >
                    <FaGithub size={24} />
                  </a>
                  <a
              ...
              <div className="mt-8 px-4">
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="text-[10px] font-black uppercase tracking-widest text-indigo-500 bg-indigo-500/10 px-4 py-1.5 rounded-lg border border-indigo-500/20"
                    >
                      {tag}
                    </span>
                  ))}
