...
const PortfolioModal = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 transition-all">
      ...
      <div className="relative bg-white dark:bg-[#16161e] w-full max-w-3xl rounded-[2.5rem] overflow-hidden border border-slate-200 dark:border-slate-800 shadow-2xl animate-in fade-in zoom-in duration-300">
        <div className="relative h-64 md:h-80">
          <img src={img} className="w-full h-full object-cover" alt={title} />
          <button
            onClick={onClose}
            className="absolute top-6 right-6 p-3 bg-white/20 dark:bg-black/40 hover:bg-red-500 backdrop-blur-md text-white rounded-full transition-colors"
          >
            <FaTimes size={20} />
          </button>
        </div>
